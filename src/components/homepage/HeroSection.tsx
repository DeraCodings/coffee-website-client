  ...
  return (
    <section className="flex h-full flex-col items-center justify-center gap-8 py-8 md:py-4 md:h-screen md:flex-row md:justify-around md:gap-1">
      <div className="flex flex-col items-center justify-center gap-8 md:pl-8 text-center">
        <h1 className="text-3xl font-extrabold text-[#443227] md:text-5xl">
          {heading?.text}
        </h1>
        <h2 className="text-xl font-light text-[#443227] md:text-2xl">
          {subheading?.text}
        </h2>
        <div>
          <Link
            href={`${cta?.href}`}
            className="rounded-sm bg-[#bf935f] px-4 py-2 font-semibold transition-colors hover:bg-[#443227] hover:text-white md:px-7 md:py-3"
          >
            {cta?.text}
          </Link>
        </div>
